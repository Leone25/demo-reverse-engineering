...
export default {
    ...
    mounted() {
        ...
        fetch(`/api/v3/books/${this.id}/bookData.json`, {credentials: 'include'}).then(res => res.json()).then(res => {
            this.bookInfo = res;
        });
    },
    beforeUnmount() {
    ...
    watch: {
        pageUrl() {
            ...
            this.pdf = usePDF({
                url: `/api/v3/books/${this.id}/${this.pageUrl}`,
            ...
            for (let i = start; i <= end; i++) {
                ...
                if (this.preloaded.has(this.bookInfo.pages[i - 1])) continue;
                fetch(`/api/v3/books/${this.id}/${this.bookInfo.pages[i - 1]}`, {credentials: 'include'}); // preload so it gets put in cache
                this.preloaded.add(this.bookInfo.pages[i - 1]);
            }
        }
    }
}
...
<template>
    ...
    <RouterLink to="/v3/library" class="back">&#60; Torna alla libreria</RouterLink>
